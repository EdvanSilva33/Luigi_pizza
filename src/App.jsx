...
function App() {
  ...
  const submitOrder = (customerData) => {
    ...
    let message = `🍕 *NOVO PEDIDO - PIZZARIA DELIVERY*\n\n`;
    ...
    message += `💳 *Forma de pagamento:* ${customerData.paymentMethod.charAt(0).toUpperCase() + customerData.paymentMethod.slice(1)}\n`;
    
    if (customerData.paymentMethod === 'dinheiro' && customerData.changeFor) {
      message += `💵 *Troco para:* R$ ${parseFloat(customerData.changeFor).toFixed(2).replace('.', ',')}\n`;
    }

    // Codificar mensagem para URL
    const encodedMessage = encodeURIComponent(message);
    const whatsappNumber = '5561998733411'; // Substitua pelo número real
